...
export async function startRecording({ 
  ...
  const outputArgs = [
    ...
    '-g', Math.max(fps * 2, 20).toString(), // Keyframe every 2 seconds (minimum 20 frames) - ensures shorter videos have keyframes
...
export async function stopRecording() {
  ...
  try {
    ...
    // Wait longer for FFmpeg to finish gracefully - critical for short recordings
    ...
    // Wait up to 10 seconds for SIGTERM to work (increased from 5)
    ...
    logger.debug('Waiting for filesystem sync and encoder flush...');
