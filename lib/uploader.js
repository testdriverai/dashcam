  ...
  if (metadata.logs && metadata.logs.length > 0) {
    ...
    for (const logStatus of trimmedLogs) {
      if (logStatus.count > 0 && logStatus.trimmedFileLocation && fs.existsSync(logStatus.trimmedFileLocation)) {
        try {
          // For CLI logs with file path mapping, use a more descriptive name
          let logName = logStatus.name || `Log ${logStatus.id}`;
          if (logStatus.type === 'cli' && logStatus.filePathMap) {
            // If there's only one file, use its path as the name
            const filePaths = Object.values(logStatus.filePathMap);
            if (filePaths.length === 1) {
              logName = filePaths[0];
            } else {
              // Multiple files tracked, use a generic name with count
              logName = `CLI Logs (${filePaths.length} files)`;
            }
          }
