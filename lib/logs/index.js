...
async function trimLogs(groupLogStatuses, startMS, endMS, clientStartDate, clipId) {
  ...
  const REPLAY_DIR = path.join(process.cwd(), 'tmp', 'recordings');
...
class LogsTrackerManager {
  ...
  async startNew({ recorderId, screenId, directory }) {
    ...
    // Ensure web daemon is running for web log tracking
    logger.debug('LogsTrackerManager: Ensuring web daemon is running...');
    await this.ensureWebDaemonRunning();
    ...
    // Note: Web logs are handled by the daemon and written to shared files
    // Recording instances don't need separate web trackers
    ...
    logger.info(`Started new logs tracker instance`, { recorderId, screenId, directory });
  ...
  async stop({ recorderId, screenId }) {
    ...
    logger.info(`Stopped logs tracker instance`, { recorderId, screenId });
    return cliStatus;
  }
