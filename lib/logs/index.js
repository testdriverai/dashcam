  ...
  groupLogStatuses.forEach((status) => {
    ...
    try {
      ...
      if (status.type === 'application' || status.type === 'cli') {
        ...
        if (status.type === 'cli') {
          // Remap logFile indices for CLI logs and store the mapping
          let map = {};
          let filePathToIndex = {};
          filteredEvents = filteredEvents.map((event) => {
            let name = map[event.logFile] ?? Object.keys(map).length + 1;
            if (!map[event.logFile]) {
              map[event.logFile] = name;
              filePathToIndex[name] = event.logFile;
            }
            return {
              ...event,
              logFile: name,
          ...
          // Store the file path mapping for use in upload
          status.filePathMap = filePathToIndex;
        }
